          ...
          <template v-for="item in options.operation.items">
            ...
            <el-dropdown v-if="item.xtype==='menu'"
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-for="subMenu in item.items" :key="subMenu.index" :command="{ command: subMenu.command, row:scope.row,  i: scope.$index }">{{ subMenu.itemText }}</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
        </template>
    ...
    <el-pagination v-if="options.pageInfo"
      ...
      <span class="pageSlot"  v-if="options.pageInfo.slotFromat">
        <template v-for="slotItem in options.pageInfo.slotFromat.split(',')">
          <span class="pageInfo yema" v-if="slotItem==='yema1'" :key="slotItem.index">页数：{{options.pageInfo.currentPage}} / {{ Math.ceil(parseInt(options.pageInfo.total) / parseInt(options.pageInfo.pageSize))}}</span>
          <span class="pageInfo pageof" v-if="slotItem==='pageof'" :key="slotItem.index">Page {{options.pageInfo.currentPage}} of {{ Math.ceil(parseInt(options.pageInfo.total) / parseInt(options.pageInfo.pageSize))}}</span>
          <span class="pageInfo zonggong" v-if="slotItem==='zonggong'" :key="slotItem.index">总共 {{options.pageInfo.total}} 条数据</span>
          <span v-if="slotItem==='space'" :key="slotItem.index">&nbsp;</span>
        </template>
      </span>
    </el-pagination>
  </el-container>
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange (val) {
      this.options.handleSelectionChange && this.options.handleSelectionChange(val)
    }
  }
}
</script>
