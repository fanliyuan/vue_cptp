    ...
    <el-pagination v-if="options.pageInfo"
      ...
      <span class="pageSlot" v-if="options.pageInfo.slotFromat&&options.pageInfo.slotFromat.length>0">
        <template v-for="slotItem in options.pageInfo.slotFromat.split(',')">
          <span class="pageInfo yema" :key="slotItem.index" v-if="slotItem==='yema1'">页数：{{options.pageInfo.currentPage}} / {{ Math.ceil(parseInt(options.pageInfo.total) / parseInt(options.pageInfo.pageSize))}}</span>
          <span class="pageInfo pageof" :key="slotItem.index" v-if="slotItem==='pageof'">Page {{options.pageInfo.currentPage}} of {{ Math.ceil(parseInt(options.pageInfo.total) / parseInt(options.pageInfo.pageSize))}}</span>
          <span class="pageInfo zonggong" :key="slotItem.index" v-if="slotItem==='zonggong'">总共 {{options.pageInfo.total}} 条数据</span>
          <span :key="slotItem.index" v-if="slotItem==='space'">&nbsp;</span>
        </template>
      </span>
    </el-pagination>
  </el-container>
