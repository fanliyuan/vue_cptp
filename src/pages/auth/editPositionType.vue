...
import editPositionService from './service/editPositionTypeService'
...
export default {
  ...
  data () {
    ...
    let delFun = (row) => {
      this.$confirm(`是否删除 ${row.nowPosition} 职位类型`).then(data => {
        if (data) {
